...
vhal_cpu_file = os.path.join(file_path, f'vhal_cpu.txt')
...
datacollector_cpu_file = os.path.join(file_path, f'datacollector_cpu.txt')

#parsed file
valuefile = spi_count_file 
cpu_file = vhal_cpu_file
valuefile_2 = datacollector_cpu_file 
...
def main():
    ...
    with open(valuefile_2,mode='r',encoding='utf-8') as f:
        content_value2 = f.readlines()
    value2_dict = {}
    ...
    for line in content_value2:
        value2_dict.update({line.split()[0]:line.split()[1]})
    ...
    while(cur_time != end_time):
        ...
        if({cur_time}.issubset(value2_dict.keys())):
            dict_result['time'] = cur_time
            dict_result['value2'] = value2_dict[cur_time]
